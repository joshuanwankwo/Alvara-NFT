...
if (!PINATA_API_KEY || !PINATA_SECRET_API_KEY) {
  ...
  console.error("Please set PINATA_API_KEY and PINATA_SECRET_API_KEY");
  process.exit(1);
}
...
async function uploadAllNamedNFTs() {
  console.log("🚀 Starting upload of named NFTs to Pinata...");
  console.log("📁 Folder: Alvara_NFTS");
  console.log("");
  ...
  const resultsPath = path.join(
    __dirname,
    "../metadata/pinata-named-nfts-results.json"
  );
  fs.writeFileSync(resultsPath, JSON.stringify(results, null, 2));

  console.log("📊 Upload Summary:");
  console.log(
    `✅ Successfully uploaded: ${results.length}/${NAMED_NFTS.length} NFTs`
  );
